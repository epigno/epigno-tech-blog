...
そこで、System76が用意しているキーボードショートカット（音量設定キー、画面の高度設定キーなど）は全部動きません。
無論、デフォルトのPopOSでは、特別な設定なく、すぐ使えます。

そこで、この記事で、画面の高度設定キーの設定を紹介します。

## あらすじ

高度設定キーは高度を変更するカスタムなコマンドにビンディングすることで、`i3`で高度設定キーが普通に使えるようになります。
まず、カスタムなバインディングを定義するため、高度設定キーのシンボルを取得する必要があります。

## 高度設定キーのシンボル(keysym)を取得

まず、キーボード上、高度設定キーを目視しましょう。

![高度設定キー](/img/system76-lemur-pro-keyboard.png)
...
`xev` を起動してから、高度ダウン・高度アップのキーを押しましょう。下記のようなテキストデータが出力されます。
...
ここで、`XF86MonBrightnessDown` および `XF86MonBrightnessUp` の高度ダウン・高度アップキーのシンボルがあります。
...
これを知った上で、高度を変更するスクリプトを書きます。

## 高度を変更するスクリプトを書く

`/usr/local/bin/backlight` に下記のスクリプトを入れます。

現高度を読み取り、それを変えるスクリプトです。
高度が必ず `1` と `max_brightness` の間にあるようにしています。
